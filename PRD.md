...
| BPM detection unreliable in real venues | Lose the most valuable audio signal | Current approach: pull BPM from AudD/Deezer metadata when a song is recognized (accurate but only when song is identified). **Next step: native audio module** (e.g. `react-native-audio-record`) to stream raw PCM and run autocorrelation/FFT beat detection on-device — gives real-time BPM even for unrecognized tracks. Requires native rebuild but is the correct long-term solution. |
