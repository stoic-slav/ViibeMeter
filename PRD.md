...
| BPM detection | AudD/Deezer metadata → ShazamKit (planned) | Music tempo and energy level | 128 BPM house set vs 80 BPM background jazz tells you more than volume. Single most underrated signal. Currently from song recognition metadata; long-term: native PCM module for unrecognized tracks |
| Song recognition + genre | AudD API → ShazamKit (planned) | Track identity and genre for context | Enables genre-segmented analysis; ShazamKit is free with Apple Developer account and has no rate limits |
...
| BPM detection unreliable in real venues | Lose the most valuable audio signal | Current: pull BPM from Deezer metadata when song is recognized (accurate, only works for identified tracks). **Step 2: migrate to ShazamKit** (free with Apple Developer account, no rate limits, native iOS). **Step 3: native PCM module** (`react-native-audio-record`) for real-time BPM on unrecognized/DJ-mixed tracks via autocorrelation — correct long-term solution. |
| AudD trial quota exhausted | Song recognition stops working silently | Migrate to ShazamKit once Apple Developer account is purchased ($99/yr, already needed for TestFlight). ShazamKit has no documented rate limits, zero per-request cost, and comparable accuracy (~99%+). Uses `expo-shazamkit` package, requires native rebuild. Drop AudD entirely post-migration. |
